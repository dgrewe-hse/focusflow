	...
	<div>
		<fieldset class="fieldset w-xs bg-base-200 border border-base-300 p-4 pt-2 rounded-box">
			...
			<form class="fieldset w-xs p-4" method="post" use:enhance>
				<p class="text-info mb-2">Your username must be at least 3 characters long and your password must be at least 8 characters long.</p>

				<label class="fieldset-label" for="form-signup.username">Username</label>
				<input class="input" placeholder="Username" id="form-signup.username" name="username" value={form?.username ?? ""} minlength="4" maxlength="31" required/>

				<label class="fieldset-label" for="form-signup.email">Email</label>
				<input class="input" placeholder="Email" type="email" id="form-signup.email" name="email" autocomplete="username" value={form?.email ?? ""} required/>
		
				<label class="fieldset-label" for="form-signup.password">Password</label>
				<input class="input" placeholder="Password" type="password" id="form-signup.password" name="password" autocomplete="new-password" required />
		...
		<a href="/login" class="p-1 link-hover">Sign in</a>
	</div>
</div>